...
@php
    $configuredCount = $productItems->filter(function ($product) {
        return ($product->mega_dealer_price ?? 0) > 0
            || ($product->dealer_price ?? 0) > 0
            || ($product->client_price ?? $product->price ?? 0) > 0;
    })->count();
@endphp
...
<form action="{{ route('products.storeBulk') }}" method="POST">
    ...
    <div class="summary-strip">
        ...
        <div class="summary-tile">
            <div class="summary-label">Configured</div>
    ...
    <div class="pricing-table-wrap">
        <table class="pricing-table">
            <thead>
                <tr>
                    <th>Item</th>
                    <th><span class="role-label"><span class="role-dot srp"></span>Item SRP</span></th>
                    ...
                    <th><span class="role-label"><span class="role-dot client"></span>Client</span></th>
                    <th>SKU</th>
                    <th>Status</th>
                </tr>
            </thead>
                ...
                @foreach($items as $index => $item)
                    ...
                    <tr class="product-row">
                        ...
                        <td>
                            ...
                            <div class="price-note"></div>
                            <input type="hidden" name="items[{{ $index }}][price]" value="{{ $clientPrice }}">
                        </td>
                        <td>
                        ...
                        <td class="status-cell">
                            @if($hasPrice)
                                <span class="status-chip">Configured</span>
                            @else
                                <span class="status-chip pending">No price</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateStatus(row) {
        ...
        if (rowHasPrice(row)) {
            statusCell.innerHTML = '<span class="status-chip">Configured</span>';
        } else {
            statusCell.innerHTML = '<span class="status-chip pending">No price</span>';
        }
    }

    function updateConfiguredCount() {
        const count = Array.from(rows).filter(rowHasPrice).length;
    ...
    if (search) {
        search.addEventListener('input', function() {
            ...
            rows.forEach(function(row) {
                ...
                const sku = row.querySelector('.product-search-sku').value.toLowerCase();
