...
<style>
    ...
    .adpo-product-tools { display: grid; grid-template-columns: minmax(220px, 1fr) 170px; gap: 8px; max-width: 500px; width: 100%; }
    ...
    .product-check { position: absolute; top: 15px; left: 35px; z-index: 2; width: 20px; height: 20px; cursor: pointer; }
    ...
    .product-image { height: 150px; display: flex; align-items: center; justify-content: center; margin-bottom: 12px; background: #f8fafc; border: 1px solid #eef2f7; border-radius: 8px; overflow: hidden; }
...
@php
    ...
    $stoveKitColors = ['white' => 'White', 'choco' => 'Choco', 'blue' => 'Blue', 'green' => 'Green'];
    $uniformSizes = [
...
<form action="{{ route('ad-purchase-orders.store') }}" method="POST" id="adpoForm">
    ...
    <div class="adpo-topbar">
        ...
        <button type="submit" class="btn btn-danger px-4"><i class="bi bi-send"></i> Submit ADPO</button>
    </div>
    ...
    <div class="adpo-sheet">
        <div class="adpo-brand">
            <div>
                ...
                <h4 class="adpo-title">Area Distributor Purchase Order</h4>
                <p class="adpo-subtitle">Build your ADPO from available active products.</p>
            </div>
            <div class="adpo-badge">
                <i class="bi bi-clipboard-check"></i> ADPO
            </div>
        </div>
        ...
        <div class="adpo-layout">
                    ...
                    <div class="adpo-info-panel">
                        <div class="adpo-info-grid">
                            ...
                            <div class="adpo-field-span">
                                ...
                                <input class="form-control" name="delivery_address" value="{{ old('delivery_address', optional($ad)->delivery_address) }}" data-uppercase>
                            </div>
                            <div class="adpo-field-span">
                ...
                <div class="adpo-section">
                    <div class="row g-3">
                        ...
                        <div class="col-lg-6">
                            ...
                            @php
                                $useRebateVoucher = old('use_rebate_voucher', old('payment_method') === 'voucher' ? 'yes' : 'no');
                            @endphp
                            ...
                            <div id="voucherCodeWrap" class="mt-3">
                                <label class="form-label">Voucher Code</label>
                                <input type="text" name="voucher_code" id="voucherCode" class="form-control" value="{{ old('voucher_code') }}" data-uppercase placeholder="Enter voucher code" @if($useRebateVoucher === 'yes') required @endif>
                                <input type="hidden" name="rebate_amount" id="rebateAmount" value="{{ old('rebate_amount', 0) }}">
                                <small class="text-muted d-block mt-1" id="voucherFeedback">The voucher discount will be calculated automatically.</small>
                            </div>
                            {{-- <div class="row g-2">
                ...
                <div class="adpo-section">
                    ...
                    <div class="product-grid" id="productGrid">
                        ...
                        @forelse($regularProducts as $product)
                            ...
                            @endphp
                            <div class="product-card {{ $oldSelected ? 'is-selected' : '' }}" data-name="{{ strtolower((string) $productName) }}" data-price="{{ $unitPrice }}" data-has-colors="{{ $isStoveKit ? '1' : '0' }}" data-has-sizes="{{ $isUniform ? '1' : '0' }}">
            ...
            <aside class="adpo-side">
                <div class="summary-card">
                    ...
                    <div class="summary-row" id="rebateSummaryRow">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function checkVoucherCode() {
        ...
        const key = voucherCode.value.trim().toUpperCase() + '|' + currentVoucherSubtotal.toFixed(2);
    ...
    function recalc() {
        ...
        const delivery = deliveryFee && selectedShippingType() === 'delivered' ? parseFloat(deliveryFee.value || '0') : 0;
        const useVoucher = selectedVoucherOption() === 'yes';
        const rebate = useVoucher ? Math.min(parseFloat(rebateAmount.value || '0'), subtotal + delivery) : 0;
        const grandTotal = Math.max(0, subtotal + delivery - rebate);
        currentVoucherSubtotal = subtotal + delivery;
    ...
    function filterProducts() {
        ...
        document.querySelectorAll('.product-card').forEach(function (card) {
            const isVisible = card.dataset.name.indexOf(term) !== -1;
        ...
        productEmptySearch.classList.toggle('is-visible', term !== '' && visibleCount === 0);
